...
full_dat <- read.csv("~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Master Data/Groundfish, sponge, coral/CCIRA Tow Video/Full Transcription/R script merging all data/rockfish counts with gear selectivity.csv",header=TRUE)
lh_traits <- read.csv("~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Master Data/Groundfish, sponge, coral/CCIRA Tow Video/Full Transcription/R script merging all data/species life histories.csv")
...
saveRDS(data_points,"~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Master Data/Groundfish, sponge, coral/Data Merger 2022/ccira_rockfish_spatial.rds")

bc_sub <- bchres %>% st_intersection(plot_area_ncc)
bc_fish <- bchres %>% st_intersection(catch_area)

#saveRDS(coast_line,"~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Master Data/Groundfish, sponge, coral/Data Merger 2022/ccira_boundary.rds")
...
ncc_coast <- ggplot() +
  ...
  geom_sf(data=data_point_labels,pch=21,fill="grey90") +
  geom_sf(data=cc_rca,fill="yellow",alpha=0.4,col=NA,lwd=0.2)+
  geom_sf_text(data=sf::st_jitter(cc_rca_labels,amount=2000),aes(label=NAME),size=2.5,colour="black", nudge_x = -25000,nudge_y = 1000,check_overlap = FALSE)+ #Add labels
...
#Save the plot
ggsave('~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Analyses/Groundfish/Spatial GLMM/Figures/ncc_rockfish_map.pdf',plot=ncc_inset,width = 6, height = 7,units='in',dpi=800)
ggsave('~/Dropbox (CCIRA)/Ecological Research - test CCIRA business acct/Analyses/Groundfish/Spatial GLMM/Figures/ncc_rockfish_map.jpeg',plot=ncc_inset,width = 6, height = 7,units='in',dpi=800)
