...
ggsave(filename="Figures/Harvest timeseries.jpeg",dpi=600,units="in",width=12,height=7)
...
ggplot(data=harvest_timeseries,aes(x=year,y=cumul_bin_harvest_pre_rca,colour=rca_cutoff)) +
  ...
  theme_minimal() +
  theme(legend.position = "top")
ggsave(filename="Figures/Harvest timeseries pre RCA.jpeg",dpi=600,units="in",width=12,height=5)
...
ggsave(filename="Figures/Harvest before and after RCA by species.jpeg",dpi=600,units="in",width=8,height=8)
