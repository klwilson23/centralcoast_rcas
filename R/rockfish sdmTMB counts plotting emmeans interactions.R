...
cpue_trends <- emmeans(fit_sdm, ~ protection_dist_scale,at = list(protection_dist_scale = rca_dist_seq),type = "response")
cpue_trends <- broom::tidy(cpue_trends,conf.int = TRUE)

rca_distkm_2 <- seq(from=0,to=50,by=5)
rca_dist_seq2 <- (-1*(rca_distkm_2-mean(rockfish$rca_distkm))/sd(rockfish$rca_distkm))
cpue_trendss2 <- emmeans(fit_sdm, ~ protection_dist_scale,at = list(protection_dist_scale = rca_dist_seq2),type = "response")
cpue_trendss2 <- broom::tidy(cpue_trendss2,conf.int = TRUE)
cpue_trendss2$rca_distkm <- rca_distkm_2

openxlsx::write.xlsx(cpue_trendss2,file="Results/cpue at distances.xlsx")

p1 <- ggplot(data=cpue_trends,aes(x=-1*rca_distkm,y=response),fill="#7fcdbb",colour="#7fcdbb") +
  geom_ribbon(data=cpue_trends,alpha=0.5,aes(x=-1*rca_distkm,ymin=conf.low,ymax=conf.high),fill="#7fcdbb",colour="#7fcdbb") +
  geom_line(data=cpue_trends,aes(x=-1*rca_distkm,y=response),colour="#7fcdbb")+
  xlab("Proximity to nearest RCA (km)") + ylab("CPUE") +
  theme_minimal() +
...
likely_interval <- qnorm(0.83) #inner 66% interval
...
p2 <- ggplot(data=df,aes(x=protection_dist_scale.trend,y=RCA_age,group=as.factor(rmax),fill=as.factor(rmax),colour=as.factor(rmax),shape=ipcc_uncertainty)) +
  ...
  geom_point(colour="black",position = position_dodge(width = 0.6)) +
  ylab("RCA Age") + xlab("Effect of proximity to nearest RCA (km)") +
  theme_minimal() + theme(legend.position="top") +
  scale_y_discrete(position = "left") +
  geom_vline(xintercept=0,lty=3)+
  annotate("text",label="More effective RCAs",x=0.70,y=6.7,size=2.75) +
  annotate("text",label="Less effective RCAs",x=-0.70,y=6.7,size=2.75) +
  coord_cartesian(clip = "off")+
  geom_segment(aes(x = 0.1, y = 6.4, xend = 0.75, yend = 6.4), 
  ...
  guides(shape="none")+
  theme(text = element_text(size=11),legend.text = element_text(size=10),strip.text=element_text(size=9,hjust = 0.35))
p2
...
ggsave(filename = "Figures/marginal effects of RCA interactions emmeans.jpeg",plot=megaP,dpi=600,units="in",height=6,width=7)
