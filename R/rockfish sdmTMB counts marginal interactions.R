...
new_grid <- expand.grid("puid"="12411",
                        ...
                        "rca_distkm" = seq(from=0,to=50,length=20),
...
p <- predict(fit_sdm, newdata = as.data.frame(new_grid),offset=log(new_grid$effort/(1-new_grid$chase_prob)),se_fit=TRUE,type="link",re_form=~0,re_form_iid=~0)

p$rmax_cat <- factor(p$rmax,levels=unique(p$rmax),labels=c("Low","Mid","High"))
# p$rmax_cat <- as.character(p$rmax_cat)
...
p_sub <- p[p$rmax_cat=="High" & p$harv_cat=="100 kg" & p$year_cat=="18-years",]
p1 <- ggplot(data=p_sub,aes(x=-1*rca_distkm,y=est,group=interaction(year_cat),colour = year_cat)) +
  geom_ribbon(data=p_sub,alpha=0.2,aes(x=-1*rca_distkm,ymin=est-1.96*est_se,ymax=est+1.96*est_se,group=interaction(year_cat),fill = year_cat)) +
  ...
  xlab("Proximity to nearest RCA (km)") + ylab("ln(CPUE)") +
...
df <- data.frame("RCA_age"=rep(c(2005,2010,2015,2020,2025)-2004,times=3),"rmax"=rep(c(0.043,0.096,0.142),each=5),broom::tidy(rca_slopes_over_time))
...
df$ipcc_uncertainty <- ifelse(df$li>0,"Very likely","Not very likely")
df$ipcc_uncertainty <- factor(df$ipcc_uncertainty,levels=c("Very likely","Not very likely"))
...
df$rmax <- factor(df$rmax,levels=rev(unique(df$rmax)))
p2 <- ggplot(data=df,aes(x=protection_dist_scale.trend,y=rmax,fill=ipcc_uncertainty,colour=ipcc_uncertainty,shape=ipcc_uncertainty)) +
  geom_segment(aes(x=li,xend=ui,y=rmax)) +
  ...
  facet_grid(rows = vars(RCA_age), switch="both")+
  ylab(expression(r[max])) + xlab("Effect of spatial protection (distance from nearest RCA)") +
  theme_minimal() + theme(legend.position="top") +
  ...
  scale_shape_manual(name="Likelihood of impact",values=c(24,22)) +
  theme(text = element_text(size=11),legend.text = element_text(size=10),strip.text=element_text(size=9))
p2
mega_plot <- ggpubr::ggarrange(p1,p2,ncol=1,nrow=2,widths=c(3,2),heights=c(1,1))

ggsave(filename = "Figures/effect of RCA interactions.jpeg",plot=p1,dpi=600,units="in",height=5,width=5.5)
...
ggplot(data=p_sub,aes(x=cumulative_catch_before_rca,y=est,group=interaction(year_cat),colour = year_cat)) +
  ...
  xlab("Cumulative commercial harvest (kg)") + ylab("ln(CPUE)") +
  theme_minimal() + theme(legend.position="top") +
  theme(text = element_text(size=11),legend.text = element_text(size=10),strip.text=element_text(size=9)) +
  scale_colour_brewer(name="RCA Age",type="qual",palette=2) +  scale_fill_brewer(name="RCA Age",type="qual",palette=2)

ggsave(filename = "Figures/effect of harvest interactions.jpeg",dpi=600,units="in",height=5,width=6)
...
ggplot(data=p_sub,aes(x=year_protected,y=est,group=interaction(rca_cat),colour = rca_cat)) +
 ...
 facet_grid(rows=vars(harv_cat),cols=vars(rmax_cat),labeller = label_bquote(cols = .(as.character(rmax_cat))~r[max],rows=.(harv_cat)))+
 xlab("RCA Age (years since protection)") + ylab("ln(CPUE)") +
 theme_minimal() + theme(legend.position="top") +
 theme(text = element_text(size=11),legend.text = element_text(size=10),strip.text=element_text(size=9)) +
...
ggsave(filename = "Figures/effect of annual trend interactions.jpeg",dpi=600,units="in",height=5,width=6)
...
ggplot(data=p_sub,aes(x=rmax,y=est,group=interaction(year_cat),colour = year_cat)) +
 ...
 xlab(expression(r[max])) + ylab("ln(CPUE)") +
 theme_minimal() + theme(legend.position="top") +
 theme(text = element_text(size=11),legend.text = element_text(size=10),strip.text=element_text(size=9)) +
 scale_colour_brewer(name="RCA Age",type="qual",palette=2) + scale_fill_brewer(name="RCA Age",type="qual",palette=2)

ggsave(filename = "Figures/effect of rmax interactions.jpeg",dpi=600,units="in",height=5,width=6)
